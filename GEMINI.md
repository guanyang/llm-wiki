# GEMINI.md — LLM Wiki Schema (for Gemini CLI)

> 本文件是 Gemini CLI 维护 Wiki 的元配置。Gemini 必须在每次会话开始时读取本文件，严格遵守以下规范。
> 本文件等价于 `AGENTS.md`，针对 Gemini CLI 的工作方式做了适配。
...
| 层 | 位置 | 所有者 | 你的权限 |
|---|---|---|---|
| Schema | `GEMINI.md` / `AGENTS.md` | 人类 + 你共同演进 | 只读 |
...
- 本文件放置在项目根目录，Gemini CLI 会在每次会话开始时自动读取 `GEMINI.md`
- Gemini CLI 使用 `read_file` / `write_file` / `edit_file` 等内置工具操作文件
- 使用 `run_shell_command` 执行 `grep`、`find` 等命令辅助巡检
- Gemini CLI 支持多轮对话，利用这一点在 ingest 流程中与用户充分讨论
- 大批量更新时，先列出变更计划，获得用户确认后再逐一执行
- 如果用户未指定操作类型，根据上下文判断是 ingest、query、lint 还是 publish
- Gemini CLI 的上下文窗口较大（1M tokens），可以一次性读取多个 wiki 页面进行综合分析
- 使用 `search_files` 工具在 wiki 中搜索关键词，辅助定位相关页面
